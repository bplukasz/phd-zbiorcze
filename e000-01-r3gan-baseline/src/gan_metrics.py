...
class GANMetricsSuite:
    """GAN evaluation suite: FID, KID, Precision/Recall and LPIPS diversity."""
    ...
    def prepare_real(self, real_loader: Iterable[BatchType], *, force_recompute: bool = False) -> None:
        ...
        for batch in real_loader:
            ...
            if (
                self.cfg.max_real_images_fid_kid is not None
                and total_for_fid_kid >= self.cfg.max_real_images_fid_kid
...
def format_metrics(metrics: Dict[str, float]) -> str:
    ...
    ordered = [
        "fid", "kid_mean", "kid_std", "precision", "recall", "lpips_diversity",
        "num_real_fid_kid", "num_real_pr", "num_fake_pr", "num_fake_lpips_pool",
    ]
    parts = []
    for key in ordered:
        if key in metrics:
...
__all__ = [
    "DependencyError", "GANMetricsConfig", "GANMetricsSuite", "TorchvisionInceptionPool3", "format_metrics",
]
