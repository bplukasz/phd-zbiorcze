...
def _update_ema(prev: Optional[float], value: float, alpha: float = 0.05) -> float:
    if prev is None:
        return float(value)
...
def _format_eta_finish_ts(seconds: float) -> str:
...
def _count_remaining_metric_evals(step: int, total_steps: int, metrics_every: int) -> int:
...
def _estimate_remaining_seconds(
    ...
    if metrics_every > 0 and metrics_elapsed_ema is not None:
        eta_sec += _count_remaining_metric_evals(step, total_steps, metrics_every) * metrics_elapsed_ema
    return eta_sec


def _load_metrics_elapsed_average(csv_path: str) -> Optional[float]:
...
def train(profile: str = "base", overrides: Optional[Dict[str, Any]] = None):
    ...
    metrics_elapsed_ema: Optional[float] = _load_metrics_elapsed_average(logs_csv_path)
    ...
    for step in range(1, cfg.steps + 1):
        ...
        sec_per_iter_ema = _update_ema(sec_per_iter_ema, iter_time)
        vram_mb = (
        ...
        if step % cfg.log_every == 0:
            eta_sec = _estimate_remaining_seconds(
                step=step,
            ...
            row = {
                ...
                "eta_human": _format_eta(eta_sec) if eta_sec is not None else "",
                "eta_finish_at": _format_eta_finish_ts(eta_sec) if eta_sec is not None else "",
            ...
            if eta_sec is not None:
                eta_txt = f"  eta={_format_eta(eta_sec)} (to {row['eta_finish_at']})"
            print(
                f"[{step:>7d}/{cfg.steps}]  "
        ...
        if metrics_suite is not None and cfg.metrics_every > 0 and step % cfg.metrics_every == 0:
            ...
            metrics_elapsed_ema = _update_ema(metrics_elapsed_ema, elapsed_metrics)
            ...
            metrics_row = {
                ...
                "eta_sec": "", "eta_human": "", "eta_finish_at": "",
                "wave_reg_total": "", "wave_mu_loss": "", "wave_std_loss": "",
                "wave_fake_mu_lh": "", "wave_fake_mu_hl": "", "wave_fake_mu_hh": "",
                "wave_real_mu_lh": "", "wave_real_mu_hl": "", "wave_real_mu_hh": "",
                "fft_reg_total": "", "fft_mu_loss": "", "fft_std_loss": "",
